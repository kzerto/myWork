...
// Симметрическая разность множеств на упорядоченных
// массивах set1 и set2 в множестве outSet
void symmetricDifference(SetAnOrderedArray set1,
                         SetAnOrderedArray set2,
                         SetAnOrderedArray *outSet) {
    ...
    while (iSet1 < set1.power && iSet2 < set2.power) {
        if (set1.data[iSet1] < set2.data[iSet2]) {
            addElement(outSet, set1.data[iSet1++]);
        } else if (iSet1 == set1.power ||
                   set1.data[iSet1] > set2.data[iSet2]) {
            addElement(outSet, set2.data[iSet2++]);
        } else {
            iSet1++;
            iSet2++;
...
//Дополнения множества на упорядоченном массиве
//set до универсума universe в множестве outSet
void addToUniverse(SetAnOrderedArray universe,
                   SetAnOrderedArray set,
                   SetAnOrderedArray *outSet) {
    differenceSets(universe, set, outSet);
}
...
int main() {
    ...
    SetAnOrderedArray U = SET(u);

    int d1[25] = {};
    SetAnOrderedArray D1 = SET(d1);
    differenceSets(A, B, &D1);

    int d2[25] = {};
    SetAnOrderedArray D2 = SET(d2);
    addToUniverse(U, B, &D2);

    int d3[25] = {};
    SetAnOrderedArray D3 = SET(d3);
    addToUniverse(U, C, &D3);
    ...
    int d5[25] = {};
    SetAnOrderedArray D5 = SET(d5);
    addToUniverse(U, D4, &D5);

    int d6[25] = {};
    SetAnOrderedArray D6 = SET(d6);
    differenceSets(D5, A, &D6);

    int d[25] = {};
    SetAnOrderedArray D = SET(d);
    symmetricDifference(D1, D6, &D);
    outputSet(D, "D");
    printf("\n");
    return 0;
}
